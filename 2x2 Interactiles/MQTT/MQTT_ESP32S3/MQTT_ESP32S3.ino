//Identical code as MQTT_ESP32C3, but with different pin allocations
/* 

Current Issues:
  [] MQTT connectivity - connecting very inconsistently
  [] There are a few versions of the debug messages in the serial monitor?
Features to add: 
  [] Use LEDs to add a loading animation while connecting to wifi & MQTT
Misc:
  [] Document how to monitor MQTT publishings
  [] Document how to monitor via web page

HOW TO RUN:
  Connect arduino IDE
  Select device XIAO ESP32S3
  Upload speed 11520
  Monitor via an MQTT client at test.mosquitto.org, port 1883
...
enum LedState { OFF, RED, ORANGE, GREEN, TEAL, BLUE, PURPLE, WHITE }; 
